...
turnover_vector <- department_stores_group %>% pull(Turnover_total)
adf_result <- adf.test(turnover_vector, alternative = c("stationary", "explosive"))

adf_result
```

O resultado do p-valor indica que devemos rejeitar a hipótese nula de que a série possui uma raiz unitária, o que sugere que há evidências estatísticas para afirmar que a série é estacionária. Portanto, a série demonstra sazonalidade, mas não possui uma tendência estocástica.

### Questão 2

**Use um método de decomposição estudado e explique os componentes, tendo
